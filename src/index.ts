...
export function ideationMessage(input: { paperCount: number; minable: string[]; missing: string[]; profileEmpty: boolean; thinCards: string[]; themeUsed: boolean; fromSelection: boolean }): string {
  const parts = [
    ...
    "(2) SYNTHESIZE — generate 3-5 research ideas in the user's language. EVERY idea must cite the specific paper(s) and the specific material it builds on (a mined assumption/limitation/future-work hint, or a card's method/reusableIdea/nextAction, or a profile question); NEVER invent paper content beyond jsonData + mined material. Patterns: (a) a limitation or fragile assumption as the opportunity, (b) method transfer onto another paper's problem or the profile focus, (c) a profile open question × a technique from the papers, (d) combining papers that share a theme (see sharedThemes), (e) a future-work hint × the user's strengths. Each idea: a short title, then SEVEN labeled elements in the user's language (Japanese labels: 背景 / 解決したい問題 / 既存手法の問題点 / 提案アイディア / 期待される成果 / 社会的インパクト / 最初の実験): (1) 背景 — the context in plain language, understandable WITHOUT having read the papers; (2) the problem it tackles; (3) the gap in existing methods, grounded in the 2-3 MOST relevant papers only — never an exhaustive enumeration; (4) the proposed idea and how it solves the problem; (5) expected outcomes, measurable where possible; (6) societal impact — concrete beneficiaries/use-cases in 1-2 sentences, no grandiose claims; (7) the smallest concrete first experiment. WRITING QUALITY — ideas are prose the user reads, NOT compressed notes: write complete grammatical sentences (in Japanese: no 体言止め fragments, no telegraphic noun-chains, no nested-parenthesis pile-ups), 1-3 full sentences per element. Cite evidence inline at the END of the supporting sentence as (slug, §locator) — never as 「採掘：…」 insertions mid-clause.",
    "(3) ASK which ideas to keep; ONLY on explicit selection call saveIdea per chosen idea (kebab-case slug, title, description = 提案アイディア + 期待される成果, motivation = 解決したい問題 + 既存手法の問題点, firstExperiment = 最初の実験, sourcePapers = the cited card slugs, themes). NEVER auto-save.",
  ];
