...
export const TOOL_DEFINITION = {
  ...
  description:
    ...
    "When the user asks for NEXT-RESEARCH IDEAS from papers (a slug list, a theme, 「さっき登録した論文」 — recover those slugs from this conversation's save results or via `list` — or 「選択した/チェックした論文」, meaning the checkboxes in the panel), call `ideate`. For the checkbox case pass NO slugs and NO theme — the stored panel selection is used automatically (it is an explicit user choice, so no confirmation step; `getSelection` shows it, `setSelection` edits/clears it). Otherwise pass slugs and/or theme (union, deduped). jsonData returns per-paper ideation material (summary/novelty/method/evaluation/limitations/reusableIdeas/nextActions/relationToMyWork/themes + arxivId) + the research profile + sharedThemes + thinCards. Follow the returned message's procedure: CONFIRM the selection first when it came from a theme (theme tags are coarse — present the resolved papers as a numbered list and let the user drop irrelevant ones before mining; explicit-slug selections need no confirmation), then MINE each kept arxivId paper via the `idea-miner` subagent (Task tool, all in one turn — full texts must never enter this conversation), then SYNTHESIZE 3-5 ideas grounded ONLY in the material (cite specific papers + the specific limitation/method/assumption/profile-question each idea builds on), each as the 8-element structure 背景/解決したい問題/既存手法の問題点/提案アイディア/期待される成果/社会的インパクト/最初の実験/参考文献 — complete publication-quality sentences, evidence cited as (slug, §locator) at sentence end, references built from the material's bibliographic fields, never telegraphic note-style — then ASK which to keep and call `saveIdea` only for explicit selections (slug, title, description, motivation, firstExperiment, sourcePapers = cited card slugs, themes). `listIdeas` (optional theme/status filter) lists saved ideas — present jsonData as a numbered list in chat; ideas have NO canvas view. `updateIdea` patches an idea (typically `status`: raw → exploring → adopted/dropped; omitted fields preserved). `deleteIdea` removes one. If `saveIdea` errors with status 409, the slug exists — ask the user: overwrite (force:true), patch (updateIdea), or rename. " +
  ...
  parameters: {
    ...
    properties: {
      kind: { type: "string", enum: ["list", "read", "save", "update", "delete", "citationTable", "relatedWork", "export", "getProfile", "setProfile", "fetchMetadata", "mergePapers", "searchPapers", "fetchFullText", "ideate", "saveIdea", "listIdeas", "updateIdea", "deleteIdea", "setSelection", "getSelection"], description: "Operation. Default: list." },
      ...
      slugs: { type: "array", items: { type: "string" }, description: "ideate: explicit card slugs to gather ideation material from (combine with theme to union both). setSelection: the checked card slugs (replaces the stored panel selection; [] clears it)." },
