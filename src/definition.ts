...
export const TOOL_DEFINITION = {
  ...
  parameters: {
    ...
    properties: {
      kind: { type: "string", enum: ["list", "read", "save", "update", "delete", "citationTable", "relatedWork", "export", "getProfile", "setProfile", "fetchMetadata", "mergePapers", "searchPapers"], description: "Operation. Default: list." },
      ...
      force: { type: "boolean", description: "save: bypass the duplicate check. ONLY set after the user explicitly confirmed they want to overwrite an existing card (e.g. \"はい、上書きでお願いします\"). Never set this silently in response to a conflict envelope." },
      ...
      limit: { type: "integer", description: "searchPapers: max candidates to return (default 10, max 25)." },
      theme: { type: "string", description: "Theme to filter by (list), or to build a citation table (citationTable) / Related Work outline (relatedWork) for. Required for citationTable and relatedWork." },
      yearFrom: { type: "integer", description: "list filter / searchPapers filter: only papers published in this year or later (e.g. for '2025年以降' / 'since 2025', pass 2025). For list, papers with no recorded year are excluded." },
